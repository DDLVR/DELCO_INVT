...
2. **Prueba la conexión manual** en el mismo shell donde corres los comandos:
   ```bash
   mysql -u usuario_base_de_datos -p -h localhost -P 3306 nombre_base_de_datos
   ```
...
3. En cPanel → **MySQL® Databases**:
   * Crea o modifica el usuario `usuario_base_de_datos` con la contraseña que usas en
     `DB_PASSWORD`.
   * Añade ese usuario a la base `nombre_base_de_datos` y asigna **All
     Privileges**.
